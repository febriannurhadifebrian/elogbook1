...
class Admin extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($user_id == 1) {
            // Jika user adalah admin, tampilkan seluruh data logbook.
            $data['logbook'] = $this->Admin_model->getLogbook();
        } else {
            // Jika user bukan admin, tampilkan hanya logbook milik mereka sendiri.
            $data['logbook'] = $this->Admin_model->getLogbookByUserId($user_nip);
        }
        ...
        $data['log'] = $this->Admin_model->getAllLogbooks();
        $data['unit'] = $this->User_model->getUnitbyNip();
        $data['logbook']['kode'] = $this->Logbook_model->getLog($user_kode);
    ...
    //     $logbookModel->updateStatus($logbook_id, $new_status);
    //     // Redirect kembali ke halaman tampilan data user
    ...
    public function close(){
        ...
        if ($this->input->post('submit_date')) {
            ...
            $data['end_date'] = $this->input->post('end_date') ?? null;
            ...
            $data['start_date'] = $this->session->userdata('start_date') ?? null;
            $data['end_date'] = $this->session->userdata('end_date') ?? null;
        ...
        if ($this->input->post('reset_filters')) {
            $this->session->unset_userdata('search');
            $this->session->unset_userdata('start_date');
            $this->session->unset_userdata('end_date');
            $this->session->unset_userdata('sort_service');
            $this->session->unset_userdata('sort_category');
            redirect('admin/close'); // Redirect to the close view after resetting
            return; // Return to prevent further execution of the method
        }
        ...
        // Konfigurasi pagination
        $config['base_url'] = base_url('admin/close');
        $config['total_rows'] = $this->Logbook_model->countAllLogbook();
        $data['total_rows'] = $config['total_rows'];
        $config['per_page'] = 5;
        ...
        // Panggil fungsi yang mengambil semua data logbook dengan parameter pencarian
        $data['logbooks'] = $this->Logbook_model->getClosedLogbooks(
            $config['per_page'],
            $data['start'],
            $data['search'],
            $data['sort_category'], // Tambahkan parameter sorting kategori
            $data['sort_service'],  // Tambahkan parameter sorting layanan
            $data['start_date'],    // Tambahkan parameter start date
            $data['end_date'] 
        );
...
// }
    ...
    public function export_excel()
    {
        ...
        $start_date = $this->session->userdata('start_date') ?? null;
        $end_date = $this->session->userdata('end_date') ?? null;

        if ($start_date && $end_date) {
            $filename = 'logbook_export_' . date('Ymd', strtotime($start_date)) . '_to_' . date('Ymd', strtotime($end_date)) . '.xlsx';
        } else {
            $filename = 'logbook_export_' . date('YmdHis') . '.xlsx';
        }

        // Get logbook data with date filtering
        $logbooks = $this->Logbook_model->getClosedLogbooks(100, 0, null, null, null, $start_date, $end_date);
        ...
        $headerStyle = $spreadsheet->getActiveSheet()->getStyle('A1:F1')->getFont()->setBold(true);
